...
frontend目录下）, 共有8个子目录，其中common-lib和svg-sprites为项目依赖的静态资源，其余6个目录均为vue的spa工程，其中devops-nav为主入口，其它子服务以iframe或amd的方式接入

系统要求
nodejs版本 8.0.0及以上

安装说明
1、打包并部署相应的vue工程 进入到src/frontend目录下
# 先全局安装yarn
npm install -g yarn
# 然后执行install
yarn install
...
最后将生成的frontend文件夹copy到__INSTALL_PATH__/__MODULE__/目录下
